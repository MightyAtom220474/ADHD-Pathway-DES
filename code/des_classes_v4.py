...
class g:
    ...
    referral_rejection_rate = 0 #0.05 # % of referrals rejected, assume 5%
    ...
    triage_rejection_rate = 0 #0.05 # % rejected at triage, assume 5%
    ...
    pack_rejection_rate = 0 #0.03 # % rejected based on pack assume 3%
    
    # QB and Observations
    target_obs_wait = 4 # QB and School obs to be completed within 4 weeks
    obs_rejection_rate = 0 #0.02 # % rejected due to obs not taking place assume 1%
    
    # MDT
    target_mdt_wait = 1 # how long did it take to be reviewed at MDT, assume 1 week
    mdt_rejection_rate = 0 #0.05 # % rejected at MDT, assume 5%
    ...
    asst_rejection_rate = 0 #0.01 # % found not to have ADHD, assume 1%
...
class Model:
    ...
    def week_runner(self,number_of_weeks):
        ...
        # list to hold weekly waiting list position
        self.df_weekly_wl = []
        ...
        while self.week_number <= number_of_weeks:
            ...
            self.triage_res = simpy.Container(
                self.env,capacity=g.triage_resource,init=g.triage_resource)
        
            self.mdt_res = simpy.Container(
                self.env,capacity=g.mdt_resource,init=g.mdt_resource)
            
            self.asst_res = simpy.Container(
                self.env,capacity=g.asst_resource,init=g.asst_resource)
                  
            # capture waiting list position at end of each week
            self.results_run_number = self.results_df['Run Number'].max()
            self.results_week_number = self.results_df['Week Number'].max()
            ...
            self.df_weekly_wl.append({'Run Number':self.results_run_number,
                                'Week Number':self.results_week_number,
                                'Triage WL':self.max_triage_wl,
                                'MDT WL':self.max_mdt_wl,
                                'Asst WL':self.max_asst_wl})
            #self.combined_wl = pd.concat(self.df_weekly_wl,ignore_index=False)
            
            g.weekly_wl_posn = pd.DataFrame.from_dict(self.df_weekly_wl#, columns=
        ...
        # reset the week number ready for next run
        self.week_number = 0
    ...
    def generator_patient_referrals(self):
        ...
        while self.referral_counter <= sampled_referrals:
            ...
            self.env.process(self.patient_pathway())
    ...
    def patient_pathway(self):
            ...
            print(f'Patient number {p.id} created')
            ...
            if self.reject_referral <= g.referral_rejection_rate:
                ...
                print(f'Patient {p.id} added, current triage wl:{g.number_on_triage_wl}')
                ...
                with self.triage_res.get(1) as triage_req:
                    ...
                    print(f'Patient number {p.id} put through triage')
                        ...
                        #print(f'Patient {p} pack sent out')
                            ...
                            #print(f'Patient {p} obs started')
                                ...
                                with self.mdt_res.get(1) as mdt_req: # request an MDT resource
                                    ...
                                    print(f'Patient number {p.id} put through mdt')
                                    ...
                                    #print(f'Patient {p} MDT completed') 
                                        ...
                                        with self.asst_res.get(1) as asst_req:
                                            ...
                                            print(f'Patient number {p.id} put through assessment')
