...
class Model:
    ...
    ##################################
            ...
            # SR comment: I've moved run capturing from here and moved it to the end as was
...
class Trial:
    ...
    def run_single(self, run):
        ...
        my_model.df_weekly_wl = pd.DataFrame(my_model.df_weekly_wl)

        my_model.df_weekly_wl['Run'] = run
    
        self.weekly_wl_dfs = []

        self.weekly_wl_dfs.append(my_model.df_weekly_wl)

        results = {"Run Number":run,
            "Week Number": my_model.week_number,
        ...
        self.weekly_wl_combined = pd.concat(self.weekly_wl_dfs)
        return results, self.weekly_wl_combined
...
df_trial_results, df_weekly_wl = my_trial.run_trial()
